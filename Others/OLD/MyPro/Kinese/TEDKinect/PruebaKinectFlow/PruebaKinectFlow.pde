...
Particle[] flow = new Particle[20000];
...
void setup() {
  ...
  randomSeed(5);
  img = new PImage();
  hint(DISABLE_DEPTH_TEST);//permite la transparencia del background
  smooth(); 
  if (!context.enableScene()) { 
    ...
    blobs = createImage(kinectWidth/6, kinectHeight/6, RGB);
...
void drawFlowfield() {
  ...
  globalX = noise(frameCount * 0.1) * width/2 + width/4;
