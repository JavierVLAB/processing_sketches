...
void setup() {
  ...
  if (!context.enableScene()) { 
    ...
    box2d.listenForCollisions(); // para ver las colisiones
...
void draw() {
  ...
  //setRandomColors(1240);
}

void updateAndDrawBox2D() {
  // if frameRate is sufficient, add a polygon and a circle with a random radius
  if (polygons.size() < 50 && frameRate > 20) {
    //polygons.add(new CustomShape(kinectWidth/2, -50, -1));
    polygons.add(new CustomShape(kinectWidth/2, -50, random(4, 20)));
  }
...
void beginContact(Contact cp) {
  ...
  if (o1.getClass() == PolygonBlob.class) {
    box2d.destroyBody(b2);
  }

  if (o2.getClass() == PolygonBlob.class) {
