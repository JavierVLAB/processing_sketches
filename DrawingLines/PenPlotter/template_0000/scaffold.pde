...
final int CONFIG_WIDTH_PIXELS = 600;
final int CONFIG_HEIGHT_PIXELS = 600;
...
final String CONFIG_GCODE_PEN_DOWN = "M03 S100\n";
...
void __saveGcode(boolean writeToFile) {
  ...
  if (writeToFile) {
    ...
    String ss = f.replace(".svg",".gcode");
    PrintWriter out = createWriter(ss);
    out.println(gcode);
    out.flush();
    out.close();
  } else {
