...
final int CONFIG_WIDTH_PIXELS = 648;
 ...
 */
int CONFIG_SCALE_FACTOR = 4;
int CONFIG_NUM_XTILES = 1;
int CONFIG_NUM_YTILES = 1;
 ...
 */
final int CONFIG_PRINT_WIDTH_MM = 210;
final int CONFIG_PRINT_HEIGHT_MM = 162;
final String CONFIG_GCODE_PEN_UP = "M03 S30\n";
final String CONFIG_GCODE_PEN_DOWN = "M03 S40\n";
final String CONFIG_GCODE_MOVE_FEEDRATE = "2200";
final String CONFIG_GCODE_RAPID_FEEDRATE = "3000";
...
void __saveGcode(boolean writeToFile) {
  ...
  for (int i = 0; i < paths.length; i++) {
    ...
    for (int j = 0; j < paths[i].length; j++) {
      ...
      float ymapped = map(paths[i][j].x, 0, width, 0, CONFIG_PRINT_WIDTH_MM);
      /* Flip Y axis since GRBL expects (0, 0) to be at the bottom left */
      float xmapped = map(paths[i][j].y, 0, height, CONFIG_PRINT_HEIGHT_MM, 0);
