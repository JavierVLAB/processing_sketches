...
void setup() {
  ...
  String path = sketchPath("../images2");
  filenames = listFileNames(path);
  //printArray(filenames);
  my_img = load_my_image("../images2/" + filenames[int(random(filenames.length))]);  
  my_original_img = my_img;
  image(my_img, 0, 0);
  //noLoop();
  strokeWeight(0.1);
}
...
void keyPressed() {
  PImage pre_img = load_my_image("../images2/" + filenames[int(random(filenames.length))]);
    //PImage pre_img = load_my_image("../images/pineapple-supply-co-prP9CYcRpzA-unsplash.jpg");
  PGraphics pg = createGraphics(pre_img.width, pre_img.height);

  my_original_img = pre_img;
